...
function resolveRequestUrl(source) {
  ...
  // Heureka XML je ve většině prostředí blokována CORS, proto vždy preferujeme backend proxy.
  if (source.parser === 'heureka-xml' && isExternal) {
...
async function fetchSource(source, { force = false } = {}) {
  ...
  if (!response.ok) {
    if (response.status === 404 && resolveRequestUrl(source).startsWith('/proxy')) {
      throw new Error(`${source.channel}: /proxy endpoint nebyl nalezen. Lokálně spusťte python3 dev_server.py; na Firebase nasměrujte /proxy na backend (např. Cloud Function).`);
    }
